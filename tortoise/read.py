...
if __name__ == '__main__':
    ...
    parser.add_argument('--candidates', type=int, help='How many output candidates to produce per-voice. Only the first candidate is actually used in the final product, the others can be used manually.', default=2)
    ...
    # Convert audio to MP3 and upload to Google Drive
    audio_file = os.path.join(voice_outpath, f"{outname}.wav")
    audio = AudioSegment.from_wav(audio_file)
    audio.export(audio_file.replace('.wav', '.mp3'), format='mp3')

    drive.mount('/content/gdrive')
    drive_path = '/content/gdrive/MyDrive/audio_files'
    os.makedirs(drive_path, exist_ok=True)
    audio_file_mp3 = os.path.join(drive_path, f"{outname}.mp3")
    os.rename(audio_file.replace('.wav', '.mp3'), audio_file_mp3)

    print(f"Audio file uploaded to Google Drive: {audio_file_mp3}")
    print(f"Download link: https://drive.google.com/file/d/{audio_file_mp3.split('/')[-1]}/view?usp=sharing")